...
@interface GongzhangtuanViewController ()<LoginViewDelegate,SearchGJSViewDelegate>
{
    NSString *_bookIdStr;
...
@implementation GongzhangtuanViewController
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"查看工长列表" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:21];
...
-(void)PressBarItemRight{
    ...
    if(!searchVC) searchVC=[[SearchGJSView alloc]initWithFrame:CGRectMake(0, 0, kMainScreenWidth, kMainScreenHeight) historyData:@"MyHistory_gongzhang.plist" fromName:@"工长"];
...
-(void)sortBtn:(UIButton *)sender{
    ...
    if(selected_mark==4) [MobClick event:@"Click_gz_collect"];   //友盟自定义事件,数量统计
...
-(void)requestForemanlist{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0266\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"sortType\":\"%ld\",\"keyword\":\"%@\",\"foremanExperience\":\"%ld\",\"currentPage\":\"%ld\",\"featuresId\":\"%ld\",\"auth\":\"%ld\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],(long)selected_mark,self.searchContent,(long)self.picture_experience,(long)self.currentPage+1,(long)self.picture_service,(long)self.picture_aut];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([dataArray count]){
        ...
        if(obj.foremanExperience==nil)
            ...
            cell.designer_express.text=[NSString stringWithFormat:@"成员：%@人",obj.teamMemberNum];
        ...
        if(obj.foremanExperience==nil)
            ...
            expresslbl.text=[NSString stringWithFormat:@"经验：%@",obj.foremanExperience];
...
//        if(selected_mark!=2){
            ...
            cell.score_start.text =[NSString  stringWithFormat:@"%.1f",[obj.popularityLevel floatValue]];
            for(int i=0;i<5;i++){
                if (i <srat_full) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GongzhangListObj *obj_=[dataArray objectAtIndex:indexPath.section  ];
//    ForemanInfoViewController *infovc=[[ForemanInfoViewController alloc]init];
//    GongzhangDetailViewController *infovc = [[GongzhangDetailViewController alloc]init];
...
-(void)pressbtnToGZYYUE:(UIButton *)sender{
    ...
    GongzhangListObj *obj=[dataArray objectAtIndex:sender.tag-kButton_Booking];
