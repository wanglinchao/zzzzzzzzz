...
- (void)viewDidLoad {
    ...
    self.reasonTV.layer.borderColor = [UIColor colorWithHexString:@"#CFCFCF" alpha:0.5].CGColor;
    ...
    self.imgBtn1.layer.borderColor = [UIColor colorWithHexString:@"#CFCFCF" alpha:0.5].CGColor;;
    self.imgBtn1.layer.borderWidth = 1;
    
    self.imgBtn2.layer.masksToBounds = YES;
    self.imgBtn2.layer.cornerRadius = 3;
    self.imgBtn2.layer.borderColor = [UIColor colorWithHexString:@"#CFCFCF" alpha:0.5].CGColor;;
    self.imgBtn2.layer.borderWidth = 1;
    
    self.imgBtn3.layer.masksToBounds = YES;
    self.imgBtn3.layer.cornerRadius = 3;
    self.imgBtn3.layer.borderColor = [UIColor colorWithHexString:@"#CFCFCF" alpha:0.5].CGColor;;
...
- (BOOL)verifyItems {
    if ([NSString isEmptyOrWhitespace:self.reasonTV.text]) {
        ...
        [TLToast showWithText:@"输入内容不能超过200个字符"];
        return NO;
    }
...
- (IBAction)clickApplyBtn:(id)sender {
    if ([self verifyItems]) {
        [self requestApplyAfterSale:self.orderIDStr];
    }
}

#pragma mark - 申请售后
-(void)requestApplyAfterSale:(NSString *)orderIDStr {
    [savelogObj saveLog:@"申请售后" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:65];
    ...
    dispatch_async(parsingQueue, ^{
        ...
        [postDict setObject:@"ID0340" forKey:@"cmdID"];
            ...
            dispatch_async(parsingQueue, ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (kResCode == 103401) {
                        ...
                        NSMutableDictionary *postDict = [[NSMutableDictionary alloc] init];
                        [postDict setObject:@"ID0325" forKey:@"cmdID"];
                        [postDict setObject:string_token forKey:@"token"];
                        [postDict setObject:string_userid forKey:@"userID"];
                        [postDict setObject:@"ios" forKey:@"deviceType"];
                        [postDict setObject:kCityCode forKey:@"cityCode"];
                        ...
                        NSString *string=[postDict JSONString];
                        NSDictionary *bodyDic = @{@"phaseOrderCode":self.orderIDStr,
                                                  @"repairReason":self.reasonTV.text,
                                                  };
...
//                        [TLToast showWithText:@"申请售后失败"];
                    } else if (kResCode == 103403) {
                        [self stopRequest];
                        customPromp =[[CustomPromptView alloc] initWithFrame:CGRectMake(0, 0, kMainScreenWidth, kMainScreenHeight)];
                        customPromp.contenttxt =@"你已经申请售后";
                        UITapGestureRecognizer *tap =[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideAction:)];
                        [customPromp addGestureRecognizer:tap];
                        [customPromp show];
//                        [TLToast showWithText:@"正在售后申请中"];
                    }
...
- (void)requestFinished:(ASIHTTPRequest *)request {
    ...
    NSString *respString = [request responseString];
    NSDictionary *jsonDict = [respString objectFromJSONString];
    if ([[jsonDict objectForKey:@"resCode"]integerValue] == 103251) {
        [TLToast showWithText:@"申请售后成功"];
        [[NSNotificationCenter defaultCenter]postNotificationName:kNCUpdateAfterSaleStatus object:nil];
    ...
    } else if ([[jsonDict objectForKey:@"resCode"]integerValue] == 103259) {
        [TLToast showWithText:@"申请售后失败"];
    }else if ([[jsonDict objectForKey:@"resCode"]integerValue] == 103253){
        [TLToast showWithText:@"你已经申请售后"];
    }
