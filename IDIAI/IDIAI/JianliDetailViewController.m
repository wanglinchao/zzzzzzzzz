...
@interface JianliDetailViewController ()<LoginViewDelegate> {
//    UITableView *_theTableView;
    PullingRefreshTableView *_theTableView;
    SupervisorListObj *_designerDetailModel;
...
@implementation JianliDetailViewController
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"查看监理详情" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:52];
    
     [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(tapPicture:) name:@"supervisior_tap" object:nil];
    ...
    _theTableView.backgroundColor = [UIColor colorWithHexString:@"#EF6562" alpha:1.0];
...
- (void)tapHeader:(UIButton *)sender {
    ...
    morevc.role_id=@"6";
    morevc.client_id=obj.supervisorId;
    ...
    if (indexPath.section == 0) {
        ...
        UITableViewCell *cell1 = [tableView dequeueReusableCellWithIdentifier:CellIdentifier1];
         if (!cell1) {
             ...
             headView.layer.borderColor = [UIColor colorWithHexString:@"EF6562" alpha:1.0].CGColor;
...
//             [_bgView addSubview:authzidlbl1];
        ...
        */      // by jiangt
        ...
        CGSize descSize = [util calHeightForLabel:_designerDetailModel.description_ width:kMainScreenWidth - 40 font:[UIFont systemFontOfSize:15]];
        float authzheight =0;
        if (_designerDetailModel.authzs.count>0) {
            authzheight =30;
        }
        _bgView.frame = CGRectMake(10, 35, kMainScreenWidth - 20, descSize.height + 180+authzheight);
        
        UIImageView *headView = (UIImageView *)[cell1 viewWithTag:104];
        headView.frame = CGRectMake(_bgView.frame.size.width/2 - 35, -20, 90, 90);
        [headView sd_setImageWithURL:[NSURL URLWithString:_designerDetailModel.supervisorLogoUrl] placeholderImage:[UIImage imageNamed:@"ic_touxiang_tk_over.png"]];
...
//        if (_designerDetailModel.authzs.count == 1) {
//            nameLabel.frame = CGRectMake((_bgView.frame.size.width - size.width)/2 - 11, headView.frame.origin.y + headView.frame.size.height + 10, size.width, 21);
...
//            NSDictionary *imgDic = [_designerDetailModel.authzs objectAtIndex:0];
//            authentIV.image = [UIImage imageNamed:[NSString stringWithFormat:@"ic_rz_b_%i.png",[[imgDic objectForKey:@"authzId"]integerValue]]];
//        } else if (_designerDetailModel.authzs.count == 2) {
//            nameLabel.frame = CGRectMake((_bgView.frame.size.width - size.width)/2 - 22, headView.frame.origin.y + headView.frame.size.height + 10, size.width, 21);
//            UIImageView *authentIV = (UIImageView *)[cell1 viewWithTag:122];
//            authentIV.frame = CGRectMake(nameLabel.frame.origin.x + nameLabel.frame.size.width + 5, nameLabel.frame.origin.y + 2, 18, 18);
//            UIImageView *authentIV2 = (UIImageView *)[cell1 viewWithTag:131];
//            authentIV2.frame = CGRectMake(nameLabel.frame.origin.x + nameLabel.frame.size.width + 5 + 18 + 3, nameLabel.frame.origin.y + 2, 18, 18);
//            NSDictionary *imgDic = [_designerDetailModel.authzs objectAtIndex:0];
//            NSDictionary *imgDic2 = [_designerDetailModel.authzs objectAtIndex:1];
//            authentIV.image = [UIImage imageNamed:[NSString stringWithFormat:@"ic_rz_b_%i.png",[[imgDic objectForKey:@"authzId"]integerValue]]];
...
//        }
        ...
        if(_designerDetailModel.experience)
            experienceStr = [NSString stringWithFormat:@"经验: %@",_designerDetailModel.experience];
        else
            experienceStr = @"经验: ";
        ...
        if(_designerDetailModel.supervisorLevel < _designerDetailModel.supervisorLevel)
            star_level=[_designerDetailModel.supervisorLevel integerValue] *2+1;
        else
            star_level=[_designerDetailModel.supervisorLevel integerValue] *2;
        ...
        if (_designerDetailModel.authzs.count>0) {
            int count =0;
            for (NSDictionary *authzdic in _designerDetailModel.authzs) {
...
//            NSDictionary *imgDic1 = [_designerDetailModel.authzs objectAtIndex:1];
...
//        }
        ...
        UILabel *descLabel = (UILabel *)[cell1 viewWithTag:107];
         ...
         descLabel.text = _designerDetailModel.description_;
      ...
      */
        ...
        priceRangeValue.text = [NSString stringWithFormat:@"%d-%d /m²",[_designerDetailModel.priceMin integerValue],[_designerDetailModel.priceMax integerValue]];
        ...
        if([_designerDetailModel.supervisorImagesPath count]>1){
            ...
            for (int i = 0; i < [_designerDetailModel.supervisorImagesPath count]; ++i) {
                @autoreleasepool {
                    ...
                    [view_sub sd_setImageWithURL:[NSURL URLWithString:[[_designerDetailModel.supervisorImagesPath objectAtIndex:i] objectForKey:@"rendreingsPath"]] placeholderImage:[UIImage imageNamed:@"bg_morentu_xq"] options:SDWebImageTransformAnimatedImage];
            ...
            self.mainScorllView.TapActionBlock = ^(NSInteger pageIndex){
                [[NSNotificationCenter defaultCenter] postNotificationName:@"supervisior_tap" object:nil];
            };
        ...
        if([_designerDetailModel.supervisorImagesPath count]!=0){
            ...
            self.lab_count.text =[NSString stringWithFormat:@"1/%d",[_designerDetailModel.supervisorImagesPath count]];
        ...
        UILabel *nameLabel=(UILabel *)[cell3.contentView viewWithTag:Kcelltag*(indexPath.section+101)+indexPath.row];
        ...
        CGSize size=[util calHeightForLabel:string_pl width:kMainScreenWidth-90 font:[UIFont systemFontOfSize:15]];
        if(size.height<20) size.height=30;
        UILabel *lab_pl=(UILabel *)[cell3.contentView viewWithTag:Kcelltag*(indexPath.section+1)+indexPath.row];
        ...
        UILabel *timeLabel=(UILabel *)[cell3.contentView viewWithTag:Kcelltag*(indexPath.section+102)+indexPath.row];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UIView *line_secon_header=[[UIView alloc]initWithFrame:CGRectMake(0, 49.5, kMainScreenWidth-20, 0.5)];
    ...
    imv.frame = CGRectMake(kMainScreenWidth-35, 15, 10, 20);
...
-(void)requestDesignerDetail{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0127\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"supervisorID\":\"%@\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],obj.supervisorId];
...
-(void)scrollviewToCurrent:(NSInteger)index{
    ...
    self.lab_count.text=[NSString stringWithFormat:@"%d/%d",index+1,[_designerDetailModel.supervisorImagesPath count]];
}
...
-(void)dealloc{
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"supervisior_tap" object:nil];
}
...
-(void)pressbtn:(UIButton *)btn{
    if (btn.tag==1001 && self.btn_shouc.selected==NO) {
        ...
        if(_designerDetailModel.supervisorId)
            [dict_ setObject:[NSString stringWithFormat:@"%@",obj.supervisorId] forKey:@"supervisorId"];
        ...
        if(_designerDetailModel.supervisorLogoUrl!=nil)
            [dict_ setObject:obj.supervisorLogoUrl forKey:@"supervisorLogoUrl"];
        if(_designerDetailModel.supervisorLevel)
            [dict_ setObject:obj.supervisorLevel forKey:@"foremanLevel"];
        if(_designerDetailModel.description_!=nil)
            [dict_ setObject:obj.description_ forKey:@"foremanDesc"];
        if(_designerDetailModel.description_!=nil)
            [dict_ setObject:obj.experience forKey:@"foremanExperience"];
        if(_designerDetailModel.address!=nil)
            [dict_ setObject:obj.address forKey:@"address"];
        if(_designerDetailModel.mobileNo!=nil)
            [dict_ setObject:obj.mobileNo forKey:@"foremanMobile"];
        ...
        if([_designerDetailModel.authzs count])
            [dict_ setObject:obj.authzs forKey:@"foremanAuthzs"];
        if([_designerDetailModel.supervisorImagesPath count])
...
//            subscribeVC.businessIDStr = [NSString stringWithFormat:@"%@",_designerDetailModel.supervisorId];
//            subscribeVC.servantRoleIdStr = @"6";
//            IDIAIAppDelegate *appDelegate = (IDIAIAppDelegate *)[[UIApplication sharedApplication] delegate];
//            [appDelegate.nav pushViewController:subscribeVC animated:YES];
//        }];
