...
@interface RefundAndAfterSaleMainViewController () <ViewPagerDataSource, ViewPagerDelegate>

@end

@implementation RefundAndAfterSaleMainViewController
...
- (UIViewController *)viewPager:(ViewPagerControllerFive *)viewPager contentViewControllerForTabAtIndex:(NSUInteger)index {
    ...
    if (index == 0) {
        RefundListViewController *refundListVC = [[RefundListViewController alloc]init];
        if (self.fromStr)
        refundListVC.fromStr = self.fromStr;
        UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:refundListVC];
        return nav;
    } else {
        AfterSaleListViewController *afterSaleListVC = [[AfterSaleListViewController alloc]init];
        if (self.fromStr)
            afterSaleListVC.fromStr = self.fromStr;
        UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:afterSaleListVC];
        return nav;
    }
}
