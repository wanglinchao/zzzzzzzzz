...
@interface DynamicViewController () <LoginViewDelegate, CommentsViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"查看进行中服务订单" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:68];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (myOrderModel.orderState == 1) {
        ...
        myOrderDetailConfirmVC.myOrderModel = myOrderModel;
...
-(void)requestDynamiclist{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0100\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"requestRow\":\"10\",\"currentPage\":\"%d\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"], self.currentPage+1];
...
- (void)clickBookStateBtn:(UIButton *)btn {
    ...
    //        //取消
    //        NSString *bookIDStr = [NSString stringWithFormat:@"%d",subscribeListModel.id];
...
- (void)gotoPayingConfirmPage:(id)sender {
    ...
    payingConfirmVC.amounts =[NSString stringWithFormat:@"%.2f",myOrderModel.orderPhraseFee];
    IDIAIAppDelegate *delegate = (IDIAIAppDelegate *)[[UIApplication sharedApplication] delegate];
