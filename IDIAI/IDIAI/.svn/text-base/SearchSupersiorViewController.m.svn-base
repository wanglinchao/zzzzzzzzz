//
//  SearchSupersiorViewController.m
//  IDIAI
//
...
#define KHISTORY_SS_DESIGNER @"MyHistory_supersior.plist"

@interface SearchSupersiorViewController ()<UITextFieldDelegate,LoginViewDelegate>
{
...
@implementation SearchSupersiorViewController
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"监理--搜索" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:42];
...
//发送记录呼叫电话信息
-(void)requestRecordCallinfo:(SupervisorListObj *)obj_{
    dispatch_queue_t parsingQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_async(parsingQueue, ^{
        ...
        //获取被叫号码
        NSString *str_called;
        if([obj_.mobileNo length]>2) str_called=obj_.mobileNo;
        else str_called=@"";
        //获取被叫号码编号
        NSString *str_called_id;
        if([obj_.supervisorId integerValue] >=0) str_called_id=[NSString stringWithFormat:@"%@",obj_.supervisorId];
        else str_called_id=@"";
        //获取设备编号
...
#warning 只有实名欠缺其他
            ...
            if([obj.state integerValue]==1){
                ...
                [btn_phone addTarget:self action:@selector(pressbtnToSub:) forControlEvents:UIControlEventTouchUpInside];
...
//          //  [cell.designer_photo setOnlineImage:obj.supervisorLogoUrl placeholderImage:[UIImage imageNamed:@"bg_morentu_xq"]];
//            if([obj.authzs count]){
//                for(int i=0;i<[obj.authzs count];i++){
//                    NSDictionary *dict=[obj.authzs objectAtIndex:i];
...
//                    if (i <srat_full) {
//                        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0 + i * (18 + 3), 5, 15, 15)];
//                        [imageView setImage:[UIImage imageNamed:@"ico_star_0.png"]];
//                        [cell.view_star addSubview:imageView];
//                        
//                    }
//                    else if (i==srat_full && srat_half!=0) {
//                        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0 + i * (18 + 3), 5, 15, 15)];
//                        [imageView setImage:[UIImage imageNamed:@"ico_star_1.png"]];
//                        [cell.view_star addSubview:imageView];
//                        
//                    }
//                    else {
//                        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0 + i * (18 + 3), 5, 15, 15)];
//                        [imageView setImage:[UIImage imageNamed:@"ico_star_2.png"]];
//                        [cell.view_star addSubview:imageView];
//                        
//                    }
//                }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==mtableview) {
        SupervisorListObj *obj=[dataArray objectAtIndex:indexPath.section];
//        SupervisiorInfoViewController *infovc=[[SupervisiorInfoViewController alloc]init];
        IDIAI3JianLiDetailViewController *infovc = [[IDIAI3JianLiDetailViewController alloc]init];
//        infovc.supersiorid=obj.supervisorId;
        infovc.obj = obj;
...
-(void)pressbtnToSub:(UIButton *)btn{
    self.selected_btn=btn.tag;
    SupervisorListObj *obj=[dataArray objectAtIndex:btn.tag-kButton_phone];
//
//    SubscribeViewController *subscribeVC = [[SubscribeViewController alloc]initWithNibName:@"SubscribeViewController" bundle:nil];
//    subscribeVC.businessIDStr = [NSString stringWithFormat:@"%@",obj.supervisorId];
...
//    } else {
//        IDIAIAppDelegate *appDelegate = (IDIAIAppDelegate *)[[UIApplication sharedApplication] delegate];
//        SupervisorListObj *obj=[self.dataArray objectAtIndex:self.selected_btn-kButton_phone];
//        [appDelegate.nav dismissViewControllerAnimated:YES completion:^{
//            SubscribeViewController *subscribeVC = [[SubscribeViewController alloc]initWithNibName:@"SubscribeViewController" bundle:nil];
//            subscribeVC.businessIDStr = obj.supervisorId;
//            subscribeVC.servantRoleIdStr = @"6";
//            IDIAIAppDelegate *appDelegate = (IDIAIAppDelegate *)[[UIApplication sharedApplication] delegate];
//            [appDelegate.nav pushViewController:subscribeVC animated:YES];
//        }];
