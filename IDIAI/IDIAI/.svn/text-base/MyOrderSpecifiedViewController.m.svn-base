...
@interface MyOrderSpecifiedViewController () <LoginViewDelegate, CommentsViewDelegate>
{
...
@implementation MyOrderSpecifiedViewController
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"查看全已完成服务订单" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:69];
...
-(void)requestOrderlist{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0101\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"orderStates\":\"%@\",\"requestRow\":\"10\",\"currentPage\":\"%d\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],self.typeStr, self.currentPage+1];
...
-(void)requestConfirmOrderOrConfirmPaying:(UIButton *)sender {
    ...
    //确认订单（合同）记日志
    if(myOrderModel.orderState == 1) [savelogObj saveLog:@"确认订单（合同）" userID:[NSString stringWithFormat:@"%ld ",[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:59];
...
-(void)requestRefusePaying:(UIButton *)sender {
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *string=[postDict JSONString];
        NSDictionary *bodyDic = @{@"orderCode":myOrderModel.orderCode};
        NSString *string02=[bodyDic JSONString];
        NSMutableDictionary *post= [[NSMutableDictionary alloc] init];
        [post setObject:string forKey:@"header"];
        [post setObject:string02 forKey:@"body"];
