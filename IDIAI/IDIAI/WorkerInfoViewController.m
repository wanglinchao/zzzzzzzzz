...
@interface WorkerInfoViewController ()<CustomScrollViewDelegate,LoginViewDelegate,CommentsViewDelegate>{
...
@implementation WorkerInfoViewController
@synthesize obj,lab_score,btn_wyp;

-(void)dealloc{
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"worker_tap" object:nil];
...
- (void)customizeNavigationBar {
    ...
    //lab_nav_title.text=obj.nickName;
...
- (void)viewDidLoad {
    ...
    [savelogObj saveLog:@"用户查看小工详情" userID:[NSString stringWithFormat:@"%ld ",(long)[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:50];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(tapPicture:) name:@"worker_tap" object:nil];
    ...
    obj.workerBrower=[NSString stringWithFormat:@"%d",[obj.workerBrower integerValue]+1];
    [self requestBrower];
...
-(void)createHeader{
    ...
    UIView *view_header = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kMainScreenWidth, CHTwitterCoverViewHeight + _topView.bounds.size.height+80)];
    view_header.backgroundColor = kThemeColor;
    
    CGSize size=[util calHeightForLabel:obj.nickName width:200 font:[UIFont systemFontOfSize:18]];
    self.worker_name = [[UILabel alloc] initWithFrame:CGRectMake((kMainScreenWidth-200)/2, 110, 200, size.height)];
    ...
    self.worker_express = [[UILabel alloc] initWithFrame:CGRectMake((kMainScreenWidth-200)/2, 100+size.height+10, 200, 20)];
    ...
    self.worker_express.text =[NSString stringWithFormat:@"%@",obj.address];
    //[view_header addSubview:self.worker_express];
    
    UIView *authen_view=[[UIView alloc]initWithFrame:CGRectMake(0, 120+size.height+80-60, kMainScreenWidth, 60)];
    ...
    if([obj.authentication_arr count]){
        ...
        for(int i=0;i<[obj.authentication_arr count];i++){
            @autoreleasepool {
                ...
                UIImageView *image_rz=[[UIImageView alloc]initWithFrame:CGRectMake((kMainScreenWidth/([obj.authentication_arr count]+1))*i+((kMainScreenWidth/([obj.authentication_arr count]+1))-30)/2, 5, 30, 30)];
                ...
                UILabel *lab_rz = [[UILabel alloc] initWithFrame:CGRectMake((kMainScreenWidth/([obj.authentication_arr count]+1))*i+((kMainScreenWidth/([obj.authentication_arr count]+1))-40)/2, 35, 40, 20)];
                ...
                UIView *line=[[UIView alloc]initWithFrame:CGRectMake((kMainScreenWidth/([obj.authentication_arr count]+1))*(i+1), 10, 0.5, 35)];
                line.backgroundColor=[UIColor colorWithHexString:@"#B5B5B8" alpha:0.5];
                [authen_view addSubview:line];
                
                if(i==[obj.authentication_arr count]-1){
                //创建关注
                UIImageView *image_gz=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ic_guanzhu_b"]];
                image_gz.frame=CGRectMake((kMainScreenWidth/([obj.authentication_arr count]+1))*(i+1)+((kMainScreenWidth/([obj.authentication_arr count]+1))-30)/2, 5, 30, 30);
                [authen_view addSubview:image_gz];
                
                UILabel *lab_gz = [[UILabel alloc] init];
                lab_gz.frame=CGRectMake((kMainScreenWidth/([obj.authentication_arr count]+1))*(i+1)+((kMainScreenWidth/([obj.authentication_arr count]+1))-80)/2, 35, 80, 20);
                ...
                lab_gz.text =[NSString stringWithFormat:@"关注%@",obj.workerBrower];
    ...
    [self.imageview_photo setOnlineImage:obj.workerIconPath placeholderImage:[UIImage imageNamed:@"ic_touxiang_tk_over.png"]];
    [view_header addSubview:self.imageview_photo];
    
    view_header.frame=CGRectMake(0, 0, kMainScreenWidth, 120+size.height+80);
    mtableview.tableHeaderView=view_header;
}

-(void)scrollviewToCurrent:(NSInteger)index{
    self.selected_picture=index;
    self.lab_count.text=[NSString stringWithFormat:@"%d/%d",index+1,[obj.workerImgPath_arr count]];
}
...
-(void)requestCommentsList{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0037\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"objectTypeId\":\"3\",\"objectId\":\"%d\",\"currentPage\":\"%d\",\"requestRow\":\"15\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],obj.workerId,1];
...
-(void)requestEvaluationOfStar:(NSString *)title star:(NSInteger)star{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        [postDict02 setObject:[NSString stringWithFormat:@"%d",obj.workerId] forKey:@"objectId"];
...
-(void)requestBrower{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0038\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"objectTypeID\":\"3\",\"objectID\":\"%d\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],obj.workerId];
...
-(void)requestCollect{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        NSString *url=[NSString stringWithFormat:@"%@/dispatch/dispatch.action?header={\"cmdID\":\"ID0039\",\"deviceType\":\"ios\",\"token\":\"%@\",\"userID\":\"%@\",\"cityCode\":\"%@\"}&body={\"objectTypeID\":\"3\",\"objectID\":\"%d\"}",kDefaultUpdateVersionServerURL,string_token,string_userid,[[[NSUserDefaults standardUserDefaults] objectForKey:cityCodeKey] objectForKey:@"cityCode"],obj.workerId];
...
-(void)requestRecordCallinfo{
    ...
    dispatch_async(parsingQueue, ^{
        ...
        if(obj.workerId >=0) str_called_id=[NSString stringWithFormat:@"%d",obj.workerId];
        else str_called_id=@"";
        //获取设备编号
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if(section==0){
        ...
        UIView *view_=[[UIView alloc]initWithFrame:CGRectMake(20, 0, kMainScreenWidth-20, 0.5)];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section<2){
        ...
        if(indexPath.section==0)
            sting_=obj.workerInfo;
        else{
            NSMutableString *string=[NSMutableString stringWithCapacity:0];
            if([obj.jobScopeName_arr count]){
                for(int i=0;i<[obj.jobScopeName_arr count];i++){
                    [string appendFormat:@"%@ ",[[obj.jobScopeName_arr objectAtIndex:i] objectForKey:@"workerTypeName"]];
                }
                sting_=string;
            }
        }
        ...
        CGSize size=[util calHeightForLabel:obj.address width:kMainScreenWidth-40 font:[UIFont systemFontOfSize:15]];
        if([obj.workerImgPath_arr count])
            return 98+size.height+270;
        else
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section<2){
        ...
        CGSize size=[util calHeightForLabel:sting_ width:kMainScreenWidth-40 font:[UIFont systemFontOfSize:18]];
        ...
        if(obj.address.length>=1) self.lab_address.text =obj.address;
        ...
        if([obj.workerImgPath_arr count]>1){
            ...
            for (int i = 0; i < [obj.workerImgPath_arr count]; ++i) {
                @autoreleasepool {
                    ...
                    [view_sub sd_setImageWithURL:[NSURL URLWithString:[[obj.workerImgPath_arr objectAtIndex:i] objectForKey:@"imgsPath"]] placeholderImage:[UIImage imageNamed:@"bg_morentu_xq"] options:SDWebImageTransformAnimatedImage];
            ...
            self.mainScorllView.TapActionBlock = ^(NSInteger pageIndex){
                [[NSNotificationCenter defaultCenter] postNotificationName:@"worker_tap" object:nil];
            };
            [cell addSubview:self.mainScorllView];
        }
        else if([obj.workerImgPath_arr count]==1){
            ...
            view_sub.clipsToBounds=YES;
            view_sub.contentMode=UIViewContentModeScaleAspectFill;
            [view_sub sd_setImageWithURL:[NSURL URLWithString:[[obj.workerImgPath_arr objectAtIndex:0] objectForKey:@"imgsPath"]] placeholderImage:[UIImage imageNamed:@"bg_morentu_xq"] options:SDWebImageTransformAnimatedImage];
        ...
        if([obj.workerImgPath_arr count]!=0){
            ...
            self.lab_count.text =[NSString stringWithFormat:@"1/%d",[obj.workerImgPath_arr count]];
        ...
        if([obj.workerImgPath_arr count])line_three.frame=CGRectMake(20, size.height+310, kMainScreenWidth-20, 0.5);
        ...
        if(![obj.workerImgPath_arr count])lab_score.frame = CGRectMake(20, size.height+60, 90, 30);
        ...
        if(![obj.workerImgPath_arr count]) self.startView.frame=CGRectMake(110, size.height+60, 160, 20);
...
-(void)pressbtn:(UIButton *)btn{
  if (btn.tag==1001 && self.btn_shouc.selected==NO) {
    ...
    if(obj.workerId)
        [dict_ setObject:[NSString stringWithFormat:@"%d",obj.workerId] forKey:@"workerId"];
    if(obj.nickName!=nil)
    ...
    if(obj.workerBrower!=nil)
          [dict_ setObject:obj.workerBrower forKey:@"workerBrower"];
    ...
    [arr_ writeToFile:_filename atomically:NO];
       ...
       [self requestRecordCallinfo];
        [savelogObj saveLog:@"用户拨打小工电话" userID:[NSString stringWithFormat:@" %d",[[[NSUserDefaults standardUserDefaults] objectForKey:User_ID] integerValue]] modelType:22];
        NSString *osVersion = [[UIDevice currentDevice] systemVersion];
        NSString *serviceNumber;
        serviceNumber=[obj.phoneNumber stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
